...
vec2 GetWindowCoord( in vec2 vUV )
{
...
vec3 GetCameraRayDir( in vec2 vWindow, in vec3 vCameraPos, in vec3 vCameraTarget )
{
...
vec3 ApplyVignetting( in vec2 vUV, in vec3 vInput )
{
...
vec3 GetSceneNormal(in vec3 vPos)
{
...
vec3 GetRayColour( in vec3 vRayOrigin, in vec3 vRayDir, out Intersection intersection )
{
...
vec3 GetRayColour( in vec3 vRayOrigin,  in vec3 vRayDir )
{
...
if __name__ == "__main__":
    ...
    shader.main_pass.channel_0 = DataChannel(lichen, filter="mipmap", vflip=True)
    shader.show()
